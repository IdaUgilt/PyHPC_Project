...
if __name__ == '__main__':
    ...
    # Make array of p we test
    p = int(sys.argv[2])
    
    # Divide into chunks
    p = np.min([p,N]) #ensures that we do not run 10 processes if we e.g. only have 5 buildings
    all_u0_chunk = np.array_split(all_u0, p)
    all_interior_mask_chunk = np.array_split(all_interior_mask, p)
    building_ids_chunk = np.array_split(building_ids, p)

    # Parallelization
    pool = multiprocessing.Pool(p) # creates p worker processes
    ...
    for bid, u, interior_mask in zip(building_ids, all_u, all_interior_mask):
        ...
        #plt.colorbar()
        #plt.savefig(f"'02613_HPC/Miniproject/Output_error_files/5_static/interim_result_images/{bid}_temperature.png")
        #plt.close()
        #plt.imshow(u, interpolation='none')
    
    timed_time = perf_counter()-t
    print(f"{p} {timed_time:.4f}")
