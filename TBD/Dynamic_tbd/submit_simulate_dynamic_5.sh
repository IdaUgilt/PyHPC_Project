#!/bin/bash
#BSUB -J simulate_dynamic_5                   # Job name
#BSUB -q hpc                               # Queue name
#BSUB -W 03:00                             # Wall-clock time
#BSUB -R "rusage[mem=1GB]"                 # Resources
#BSUB -o simulate_dynamic_5_%J.out            # Output file (stdout)
#BSUB -e simulate_dynamic_5_%J.err            # Output file (stderr)
...
echo "n_proc time" > timings_dynamic_5.txt

for p in 1 2 4 8 16 24 32
do
    numactl --interleave=all python simulate_dynamic_5.py 100 $p >> timings_dynamic_5.txt
done